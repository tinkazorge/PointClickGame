...
volgorde items aan te klikken en puzzels op te lossen. Bij mijn spel is dit doel het laten ontsnappen van de hoofdpersoon 
...
Verder zijn er drie handlersklassen die functies bevatten waar alle activities gebruik van moeten kunnen maken. Ten eerste is er SpriteHandler,
die alle functies bevat die de bewegingen van de sprites uitvoeren. Daarnaast is er TextHandler, die de functies bevat die de teksten laten oppoppen
...
heeft gebruikt en als dit niet het geval is de geluiden afspeelt. 
...
#### Timing
Ik veel tijd besteedt aan de timing, die ik met booleans in elkaar heb gezet, wat soms nog wel zorgde voor een ingewikkelde constructie. Wanneer een bepaalde actie is uitgevoerd, gaat er een bepaalde boolean op true die zorgt dat andere acties mogelijk zijn. Dit is het uitgangspunt van de game (net als van de meeste point en click games). Het is een uitdaging om de code overzichtelijk te houden wanneer bijvoorbeeld op meerdere booleans tegelijk moet worden gecheckt in een onClickListener.

#### Besturing
Eén van de dingen die me het meeste tijd heeft gekost is het proberen te implementeren van functionaliteiten die op een veel gemakkelijker manier konden worden gedaan, zoals het besturingssysteem. Zo ben ik en tijd bezig geweest met het laten bewegen van de hoofdsprite door de x en y locatie proberen te verkrijgen. Uiteindelijk bleek het gemakkelijker te zijn om gewoon de margins van de sprite op te vragen en deze te updaten wanneer de gebruiker op de pijltjestoetsen klikt. Ik wilde eigenlijk dat de sprite naar elke plek zou lopen waar de gebruikerop tikte in het scherm, maar heb uiteindelijk pijltjestoetsen gekozen omdat deze voor de gebruiker minder frustrerend zijn om mee te werken. Wat als de gebruiker naar een plek wil lopen waarvan ik niet wil dat hij hierheen loopt? de pijltjestoetsen schelen een hoop onnodige kliks op allerlei plekken. 
...
#### API
Van te voren was ik van plan een API te gaan gebruiken zoals Unity of AndEngine, maar dit vond ik uiteindelijk niet nodig, al had ik wel al flink wat tijd besteed aan AndEngine downloaden en in mijn Andoid studio zetten. Voor animaties was het uiteindelijk misschien toch handig geweest, maar zonder API en met alleen Android Studio ben ik een heel eind gekomen. 

### Beslissingen
Er zijn een aantal dingen die ik anders heb gedaan dan ik in het begin had gedacht. Zo was mijn MVP één level, maar het leek me beter om twee levels te maken aangezien het niet handig was om te veel clickable objects in een scherm te plaatsen, anders zouden ze overlappen en werd het spel moeilijker te spelen. Daarnaast is mijn klasse-indeling totaal veranderd. Op de manier waarop ik het nu doe doe kunnen alle activities op een overzichtelijke manier gebruik maken van functies uit de Handlerklassen. Ten slotte heb ik het idee van animaties geschrapt, omdat het te veel werk zou zijn in een korte tijd om een mooie animatie te creeëren; hier was een API misschien handig voor geweest. Ook heb ik hierboven al het besturingssysteem genoemd: Ik wilde de sprite slechts laten bewegen op een horizontale lijn en hier vindt ik een touchListener waarbij de gebruiker overal in het scherm kan klikken niet bij passen, omdat de sprite niet over het hele scherm kan lopen. 
